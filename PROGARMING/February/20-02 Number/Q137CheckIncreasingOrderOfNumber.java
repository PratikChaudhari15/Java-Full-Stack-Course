...
class Q137CheckIncreasingOrderOfNumber {
    public static void main(String[] args) {
        System.out.println("Enter Number: ");
        int num = new Scanner(System.in).nextInt(); // Read input number from user
        int flag = 1; // Flag variable to track if the number is in increasing order
        ...
        while (num >= 10) { // Continue until the number has at least two digits
            ...
            if ((num % 10) < ((num / 10) % 10)) { // If current digit is smaller than the previous one
                flag = 0; // Mark as not in increasing order
        ...
        if (flag == 0) {
            System.out.println("No"); // Number is NOT in increasing order
        } else {
            System.out.println("Yes"); // Number is in increasing order
        }
    }
}
